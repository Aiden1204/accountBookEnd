...
router.post('/submitLog', async (ctx, next) => {
    ...
    if(flag){
        ...
        // 将用户提交的记录写入数据库
        await client.query("INSERT INTO log (userId,category,categoryName,money,remarks) VALUES (?,?,?,?,?);", [userId,data.category,data.categoryName,data.money,data.remarks]).then(function(result) {
